...
class MainApp extends React.Component {
  ...
  regularCount() {
    ...
    const newData = this.state.data.map((b) => {
      let res = b;
      if (b.uid == active) {
        res.count += 5;
        if (res.count >= 100) {
          ...
          axios
            .get(
              `https://maker.ifttt.com/trigger/${b.action}/with/key/${process.env.REACT_APP_IFTTT_KEY}`
            )
            .then((res) => {
              console.log(res);
            });
  ...
  componentDidMount() {
    ...
    axios.get(process.env.REACT_APP_CONFIG).then((res) => {
  ...
  render() {
    ...
    const VisableButtons = this.state.data.map((b) => (
      <UsableButton name={b.name} uid={b.uid} active={active} count={b.count} />
    ));
    return (
      <div id="container">
        <h1>Visuabilty</h1>
